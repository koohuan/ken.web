...
$a = public_path().'/assets/pQp.css';
if(!file_exists($a))
	file_put_contents($a,file_get_contents(__DIR__.'/css/pQp.css'));
$cssUrl = '/assets/pQp.css';
...
if($log){
	$output['queryTotals']['count'] = 0;
	$i = 0;
	foreach($log as $v){ 
		$output['queries'][$i]['sql'] = $v['sql'];
		$output['queries'][$i]['time'] = $v['time'];
		$output['queryTotals']['count']++; 
		$output['queryTotals']['time'] += $v['time'];
		$i++;
	}
}
