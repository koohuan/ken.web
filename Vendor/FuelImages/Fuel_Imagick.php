 ...
 */ 
include dirname(__FILE__).'/Fuel_Driver.php'; 
class Fuel_Imagick extends Driver
{
	...
	protected function _watermark($filename, $position, $padding = 5)
	{
		...
		$wmimage = new Fuel_Imagick();
		$wmimage->readImage($filename);
		$wmimage->evaluateImage(Fuel_Imagick::EVALUATE_MULTIPLY, $this->config['watermark_alpha'] / 100, Fuel_Imagick::CHANNEL_ALPHA);
		$this->imagick->compositeImage($wmimage, Fuel_Imagick::COMPOSITE_DEFAULT, $x, $y);
	}
	...
	protected function _mask($maskimage)
	{
		...
		$this->imagick->compositeImage($wmimage, Fuel_Imagick::COMPOSITE_COPYOPACITY, 0, 0);
	}
	...
	{
		$this->imagick->setImageType(Fuel_Imagick::IMGTYPE_GRAYSCALEMATTE);
	}
	...
	protected function add_background()
	{
		if($this->config['bgcolor'] != null)
		{
			...
			$tmpimage->compositeImage($this->imagick, Fuel_Imagick::COMPOSITE_DEFAULT, 0, 0);
			$this->imagick = $tmpimage;
		}
	}
	...
	protected function create_color($hex, $alpha)
	{
		...
		return new Fuel_ImagickPixel('rgba('.$red.', '.$green.', '.$blue.', '.round($alpha / 100, 2).')');
	}
}
