...
abstract class Fuel_Driver
{
	...
	/**
	 * Saves the file in the original location, adding the applicationend and prepend to the filename.
	 *
	 * @param   string   $applicationend       The string to applicationend to the filename
	 ...
	 * @return  Image_Driver
	 */
	public function save_pa($applicationend, $prepend = null, $extension = null, $permissions = null)
	{
		$filename = substr($this->image_filename, 0, -(strlen($this->image_extension) + 1));
		$fullpath = $this->image_directory.'/'.$applicationend.$filename.$prepend.'.'.
